function pictures = PSI_plot_exciton_state(state)
    Hamiltonian = load('PSI_Hamiltonial.dat');
...
%     site_coeff=Eigenv(:,energy_state);
    ...
    site_population = Eigenv(:,state);
    pictures=figure(state);
    % Error = 0;
    % clf;
    hold on
    ...
    %         Error = 0;
    %     end
        if site_population(j) >= 0;
            plot(rMg(j,1),rMg(j,2),'.','MarkerSize',site_population(j)*120 ,'MarkerEdgeColor','r');
        else
            plot(rMg(j,1),rMg(j,2),'.','MarkerSize',site_population(j)*-120 ,'MarkerEdgeColor','b');
        end
    end
    ...
    %     text(MG_coord(i,1)+2,MG_coord(i,2)+5,num2str(i),'FontSize',18);
    end
    
    plot(rMg(1,1),rMg(1,2),'*k','MarkerSize',20);
    text(rMg(1,1)+2,rMg(1,2)+5,'RC','FontSize',18);
    plot(rMg(2,1),rMg(2,2),'*k','MarkerSize',20);
end
